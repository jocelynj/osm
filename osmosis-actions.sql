...
CREATE OR REPLACE FUNCTION osmosisUpdate_way1() RETURNS void AS $$
DECLARE
  line RECORD;
BEGIN
  ...
  LOOP
    ...
    DELETE FROM relation_members WHERE member_type = 'W' AND member_id = line.id;

    DELETE FROM ways WHERE id = line.id;
    DELETE FROM way_nodes WHERE way_id = line.id;

    DELETE FROM nodes USING way_nodes WHERE nodes.id = way_nodes.node_id AND
    DELETE from actions WHERE data_type = 'W' AND id = line.id AND action != 'D';
  END LOOP;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION osmosisUpdate_way2() RETURNS void AS $$
DECLARE
  line RECORD;
BEGIN
  FOR line IN SELECT actions.id AS id
...
CREATE OR REPLACE FUNCTION osmosisUpdate_way3() RETURNS void AS $$
DECLARE
  line RECORD;
BEGIN
  FOR line IN SELECT actions.id AS id
...
SELECT * FROM osmosisUpdate_way1();
SELECT * FROM osmosisUpdate_way2();
SELECT * FROM osmosisUpdate_way3();
